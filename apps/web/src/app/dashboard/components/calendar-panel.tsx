...
export function CalendarPanel() {
  ...
  const linkClass =
    "inline-flex min-h-8 items-center gap-2 rounded-lg px-2 text-xs font-medium text-text-tertiary transition hover:bg-surface-hover hover:text-text-primary focus-visible:outline-2 focus-visible:outline-accent";
  const buttonClass =
    "inline-flex min-h-9 items-center justify-center gap-2 rounded-xl bg-text-primary px-3.5 text-[13px] font-semibold text-background shadow-[inset_0_1px_0_rgba(255,255,255,.45)] transition duration-200 hover:-translate-y-px hover:opacity-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent disabled:opacity-50";
  return (
    <DashboardCard className="min-h-[304px]">
      <SectionHeader
        eyebrow="Schedule"
        title="Upcoming"
        action={
      ...
      {loading ? (
        <CalendarSkeleton />
      ) : state === "disconnected" || state === "reconnect_required" ? (
        <div className="flex min-h-52 flex-col justify-center">
          <p className="max-w-xs text-[13px] leading-5 text-text-secondary">
            {state === "reconnect_required"
              ? "Calendar access needs your attention."
              : "Connect your calendar to see classes and commitments here."}
          </p>
          <button
        ...
        <div className="flex min-h-52 flex-col justify-center">
          <p className="text-[13px] text-text-secondary">
            Calendar couldn&apos;t be updated just now.
          </p>
          <button
            className={`${linkClass} mt-3 w-fit`}
            onClick={() => void refresh()}
          >
            Try again
          </button>
        ...
        <div>
          ...
          <CalendarTimeline events={data.events.slice(0, 5)} />
        </div>
      ) : (
        <div className="flex min-h-52 items-center text-[13px] text-text-secondary">
...
function CalendarTimeline({ events }: { events: TodayEvent[] }) {
  return (
...
function CalendarSkeleton() {
  return (
    <div className="space-y-4 pt-5">
      <div className="h-3 w-24 animate-pulse rounded-md bg-surface-hover" />
      <div className="h-10 w-full animate-pulse rounded-md bg-surface-hover" />
      <div className="h-10 w-full animate-pulse rounded-md bg-surface-hover" />
      <div className="h-10 w-full animate-pulse rounded-md bg-surface-hover" />
    </div>
  );
