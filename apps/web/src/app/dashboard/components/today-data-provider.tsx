...
export function TodayDataProvider({
  ...
  async function refresh(): Promise<void> {
    ...
    try {
      const response = await apiFetch(
        `/api/today?date=${date}&timezone=${encodeURIComponent(timezone)}`,
        {
          cache: "no-store",
        },
      );
      ...
      setData(current);
      if (
        current.calendar.state === "current" ||
        current.calendar.state === "saved"
      ) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(calendarUpdatedEvent, listener);
    return () => window.removeEventListener(calendarUpdatedEvent, listener);
  }, [userId]);

  return (
    <TodayDataContext value={{ data, loading, refreshing, refresh }}>
      {children}
    </TodayDataContext>
  );
}
